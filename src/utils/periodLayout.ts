...
export function entriesFor(entries: Entry[], referenceType: Entry['referenceType'], targetDate: string): EntryOccurrence[] {
	return entries.flatMap((entry): EntryOccurrence[] => {
		...
		if (entry.type === 'migrated_task' && entry.primaryTargetDate !== null && entry.secondaryTargetDate !== null) {
			...
			if (entry.primaryTargetDate === targetDate) {
				occurrences.push({ entry, migrationDisplay: 'original' })
			}
			if (entry.secondaryTargetDate === targetDate) {
				occurrences.push({ entry, migrationDisplay: 'current' })
			}
			return occurrences
		}
...
function sortOccurrences(occurrences: EntryOccurrence[]): EntryOccurrence[] {
